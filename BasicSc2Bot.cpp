...
bool BasicSc2Bot::TryBuildHatchery() {
  ...
  if (drone_count >= threshold_drone_count &&
      observation->GetMinerals() >= 300) {

    // Find a suitable Drone to build the Hatchery.
    const Unit *builder_drone = FindAvailableDrone();
    if (builder_drone) {
      std::cout << drone_count << " building hatchery " << threshold_drone_count
                << std::endl;
      const GameInfo &game_info = Observation()->GetGameInfo();

      Point2D loc = FindRandomLocation(game_info);

      Actions()->UnitCommand(builder_drone, ABILITY_ID::BUILD_HATCHERY, loc);
      return true;
    }
  }
  return false;
...
bool BasicSc2Bot::TryBuildSpawningPool() {
    ...
    if (observation->GetMinerals() >= 200) { // Spawning Pool costs 200 minerals
        const Unit* builder_drone = FindAvailableDrone();
        if (builder_drone) {
            Point2D build_location = FindRandomLocation(Observation()->GetGameInfo());
            Actions()->UnitCommand(builder_drone, ABILITY_ID::BUILD_SPAWNINGPOOL, build_location);
            return true;
        }
    }
    return false;
...
const Unit *BasicSc2Bot::FindAvailableDrone() {
  ...
  for (const auto &unit : units) {
    ...
    if (unit->unit_type == UNIT_TYPEID::ZERG_DRONE &&
        last_order.ability_id == ABILITY_ID::HARVEST_GATHER) {
      Actions()->UnitCommand(unit, ABILITY_ID::STOP);
      return unit;
    }
  }
...
bool BasicSc2Bot::TryBuildDrone() {
  ...
  // Ideal worker count per Hatchery is 16 on minerals, plus gas workers if
  // applicable.
...
//         Actions()->UnitCommand(zergling, ABILITY_ID::ATTACK, enemy_base_location);
//     }
// }